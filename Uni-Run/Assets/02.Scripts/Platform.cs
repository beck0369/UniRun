using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Platform : MonoBehaviour   // 발판으로서 필요한 동작을 담은 스크립트
{
    public GameObject[] obstacles;  // 장애물 오브젝트를 담을 변수 지정
    private bool stepped = false;   // 플레이어 캐릭터가 밟았는지 체크하는 변수 지정

    private void OnEnable() // 컴포넌트가 활성화될 때마다 매번 실행되는 메서드
    {
        // 발판을 리셋하는 처리
        stepped = false;

        for (int i = 0; i < obstacles.Length; i++) // 장애물의 수만큼 루프
        {
            if (Random.Range(0, 3) == 0)    // 현재 순번의 장애물을 1/3의 확률로 활성화
            {
                obstacles[i].SetActive(true);
            }
            else
            {
                obstacles[i].SetActive(false);
            }
        }
    }

    private void OnCollisionEnter2D(Collision2D collision)  // 플레이어 캐릭터가 자신을 밟았을 때 점수를 추가하는 처리
    {
        if (collision.collider.tag == "Player" && !stepped) // 충돌한 상대방의 태그가 Player이고, 이전의 플레이어 캐릭터가 밟지 않았다면
        {
            // 점수를 추가하고 밟힘 상태를 참으로 변경
            stepped = true;
            GameManager.instance.AddScore(1);
        }
    }
}
